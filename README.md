...
**短链跳转流程**：布隆过滤器判断 → Caffeine 本地缓存 → Redis 缓存 → 未命中查数据库 → 校验有效性（过期/状态）→ 计数 +1 → 302 跳转

## 性能压测

压测环境：本地单实例，8 核，Java 17 + Spring Boot 4 + PostgreSQL + Redis；工具 [hey](https://github.com/rakyll/hey)，原始 URL 使用占位串（未压测真实站点）。

| 接口 | 并发 | QPS | P50 | P99 | 结果 |
|------|------|-----|-----|-----|------|
| 跳转（读，Caffeine+Redis 缓存命中） | 50 | 4,839 | 8ms | 43ms | 100% 302，零错误 |
| 跳转（读） | 100 | 5,486 | — | 67ms | 100% 302，零错误 |
| 创建（写，双 buffer 号段 + Hashids） | 30 | 2,444 | 11ms | 37ms | 零重复短码 |
| 二维码生成（ZXing PNG） | 30 | 1,515 | — | 150ms | 100% 200 |

**并发正确性**：创建接口高并发压测跨约 100 次号段切换，累计发号 10 万级，短码零重复（数据库唯一约束 + `COUNT(DISTINCT)` 校验通过）。

**优化对比**：号段发号器由单 buffer 升级为双 buffer 后，创建接口 QPS 从 ~1,980 提升至 ~2,444、P99 从 43ms 降至 37ms；跳转接口引入 Caffeine 本地缓存后，QPS 从 ~3,800 提升至 ~4,800。
